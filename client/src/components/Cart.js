...
export default function Cart() {
    ...
    const handleCheckOut = async () => {
        ...
        var userEmail = dataEmail.email;

        let response = await fetch("http://localhost:8000/display/orderData", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
                    ...
                    <tbody>
                        {data.map((food, index) => (
                            <tr>
                                ...
                                <td>{food.size}</td>
                                <td>{food.price}</td>
                                <td ><button type="button" className="btn p-0"><i class="zmdi zmdi-delete" onClick={() => { dispatch({ type: "REMOVE", index: index }) }}></i></button> </td></tr>
                        ))}
                    </tbody>
                </table>
